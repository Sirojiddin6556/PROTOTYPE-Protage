...
from pbz import query as que, creation as crv, edit as edv
from ontolog import OnClass as oCl, OnObjPropety as oP

# СОздание глобального класса приложения
...
# создание главного окна инерфэйса

    def __init__(self):
        super().__init__()
        main_menu = Menu(self)
        main_menu.add_command(label='Loading', command=self.load_ontology)
        main_menu.add_command(label='Save', command=self.save_ontology)
        ...
        main_menu.add_command(label="Adding", command=self.creation_window)
...
# создание виджетов каждой кнопке и настраиваем кнопки

        self.tab_label = Label(self, text="Current page:")
        ...
        vocabulary_frame = Frame(self.notebook, bd=2, bg="blue")
        obj_prop_frame = Frame(self.notebook, bd=2, bg="blue")
        subj_pred_obj_frame = Frame(self.notebook, bd=2, bg="blue")
        individuals_tree_frame = Frame(self.notebook, bd=2, bg="blue")
...
# функция для добавлления свойств атрибутов
    ...
    def open_individuals_attributes(self, event):
        ...
        ind_frame = Frame(ind_attr_win, bd=2, bg="blue")
...
# функция обновление свойст объекта
...
# функция создание опорного узела
